...
struct VIMemory
{
	UVIVerticalTimingRegister vertical_timing;
	UVIDisplayControlRegister display_control;
	UVIHorizontalTiming0 htiming0;
	UVIHorizontalTiming1 htiming1;
	...
	UVIFBInfoRegister xfb_info_top_3d;      // Start making your stereoscopic demos! :p
	UVIFBInfoRegister xfb_info_bottom;
	UVIFBInfoRegister xfb_info_bottom_3d;
	u16 vbeam_pos; // 0: Inactive
	u16 hbeam_pos; // 0: Inactive
	UVIInterruptRegister interrupt[4];
	UVILatchRegister latch[2];
	UVIHorizontalStepping horizontal_stepping;
	UVIHorizontalScaling horizontal_scaling;
	SVIFilterCoefTables filter_coef_tables;
	u32 unk_aa;// ??? 0x00FF0000
	u16 clock;        // 0: 27MHz, 1: 54MHz
	UVIDTVStatus dtv_status;
	u16 fbwidth;      // Only correct when scaling is enabled?
